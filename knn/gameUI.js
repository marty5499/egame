...
class GameUI {
  ...
  constructor(config) {
    ...
    this.zoomX = this.canvas.width / (config.maxX != undefined ? config.maxX : 10);
    this.zoomY = this.canvas.height / (config.maxY != undefined ? config.maxY : 10);
  }
  ...
  drawCrossLine() {
    ...
    for (var x = 0; x <= this.cw; x = x + this.zoomX) {
      for (var y = 0; y <= this.ch; y = y + this.zoomY) {
        this.ctx.rect(x, y, x + this.zoomX, y + this.zoomY);
      }
    }
    this.ctx.stroke();
    this.ctx.closePath();
  ...
  cloneObj(obj, x, y) {
    ...
    //console.log(obj.x+ ','+obj.y,' -> ',newObj.x+','+newObj.y);
    return newObj;
  }
  ...
  drawLineWithArrows(objA, objB, aWidth, aLength, arrowStart, arrowEnd, color) {
    ...
    var x0 = objA.x * this.zoomX;
    var y0 = objA.y * this.zoomY;
    var x1 = objB.x * this.zoomX;
    var y1 = objB.y * this.zoomY;
    var dx = x1 - x0;
    var dy = y0 - y1;
  ...
  async loadImage(url, width, height) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.canvas.parentNode.appendChild(img);
      img.src = url;
      img.width = width;
      img.height = height;
