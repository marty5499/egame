var dt = (function() {
    ...
    function buildDecisionTree(builder) {
        ...
        for (var i = trainingSet.length - 1; i >= 0; i--) {
            ...
            for (var attr in item) {
                ...
                var predicateName = builder.condition[attr];
                /*
                if (!isNaN(pivot)) {
                    predicateName = '>=';
                } else {
    ...
    function predict(tree, item) {
        ...
        while (true) {
            if (tree.category) {
                tree.attr = tree.category;
                path.push('tree.category'); // only leafs contains predicted category
                console.log('check:', tree.category);
            ...
            //path.push(attr);
            //path.push(value >= pivot ? "Yes" : "No");
            tree.attr = attr;
            tree.mark = eval(value + tree.predicateName + pivot);
            //*
            console.log("Check : (", attr, value, tree.predicateName,
                pivot, ') result:', tree.mark);
            //*/
