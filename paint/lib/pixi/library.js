...
class PIXI_Shape {
    ...
    drawPolygon(data) {
        this.polygon = new PIXI.Polygon(data);
    ...
    calculate(ctx, minX, minY) {
        //debugger;
        //var pixels = this.gameObj.app.renderer.plugins.extract.pixels(this.g);
        //console.log("w:", this.g.width, " , h:", this.g.height);
        var pixels = ctx.getImageData(0, 0, 640, 480).data;
        //console.log(pixels);
        var h = parseInt(this.g.height);
        var w = parseInt(this.g.width);
        var area = 0;
        var inside = 0;
        var out = 0;
        var bad = 0;
    ...
    createPolygon(side, r) {
        ...
        for (let i = 0; i < sides - 1; i++) {
            ...
            const x = parseInt(radius * Math.cos(theta));
            const y = parseInt(radius * Math.sin(theta));
